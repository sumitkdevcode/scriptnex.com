  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-2 px-3 py-1.5 bg-white/[0.04] border border-[#2a2d35] rounded-md text-[#ababab] text-sm hover:border-[#ababab] hover:text-white transition-colors"
      ...
      {open && (
        ...
        >
          <div className="w-full max-w-2xl bg-[#16181d] border border-[#2a2d35] rounded-md shadow-2xl overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-3 border-b border-[#2a2d35]">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#ababab" strokeWidth="2">
                <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
              </svg>
              <input
                autoFocus
                ...
                className="flex-1 bg-transparent text-white placeholder-[#ababab] outline-none text-base"
              />
              {loading && <div className="animate-spin rounded-full h-4 w-4 border-t-2 border-[#00d285]" />}
              <button onClick={close} className="text-[#ababab] hover:text-white text-xs">ESC</button>
            </div>

            <div className="max-h-[60vh] overflow-y-auto">
              {!query && (
                <div className="px-4 py-8 text-center text-[#ababab] text-sm">
              ...
              {results?.problems.length ? (
                <div className="px-2 py-2">
                  <div className="px-3 py-1.5 text-[10px] uppercase tracking-widest text-[#ababab] font-semibold">Problems</div>
                  ...
                  {results.users.map((user) => (
                    <Link key={user.id} href={`/${user.username}`} onClick={close}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-[#ababab]">@{user.username}</div>
                      </div>
                    </Link>
                  ))}
                </div>
              ...
              {results?.contests.length ? (
                <div className="px-2 py-2 border-t border-[#1a1c23]">
                  <div className="px-3 py-1.5 text-[10px] uppercase tracking-widest text-[#ababab] font-semibold">Contests</div>
