...
export default function ContestDetailClient({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    api.get<{ contest: ContestDetail; problems: ContestProblem[] }>(`/contests/${slug}`)
  ...
  return (
    <div className="min-h-screen bg-[#0f1115] text-[#f8fafc]">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <Link href="/contests" className="text-sm text-[#ababab] hover:text-white mb-4 inline-block">← Back to Contests</Link>
        ...
        <div className="flex gap-6 text-sm text-[#ababab] mb-6">
        ...
        {contest.description && <p className="text-[#ababab] mb-8 whitespace-pre-wrap">{contest.description}</p>}

        {contest.status !== 'ended' && (
          <button onClick={handleRegister} disabled={registered} className={`px-8 py-3 rounded-xl font-bold text-sm mb-10 transition-colors ${registered ? 'bg-[#2a2d35] text-[#ababab] cursor-default' : 'bg-[#00d285] text-black hover:bg-[#00e691]'}`}>
        ...
        {contest.rules && (
          <div className="bg-[#16181d] border border-[#2a2d35] rounded-xl p-6 mb-6">
            <h2 className="text-sm font-semibold uppercase tracking-widest text-[#ababab] mb-3">Rules</h2>
            <pre className="text-sm text-[#ababab] whitespace-pre-wrap">{contest.rules}</pre>
          </div>
        )}
        ...
        {problems.length > 0 && (
          <div className="bg-[#16181d] border border-[#2a2d35] rounded-xl overflow-hidden">
            <div className="px-6 py-3 border-b border-[#2a2d35]">
              <h2 className="text-sm font-semibold uppercase tracking-widest text-[#ababab]">Problems</h2>
            </div>
            {problems.map((p, i) => (
              <Link key={p.id} href={`/problems/${p.slug}`} className="flex items-center justify-between px-6 py-3.5 hover:bg-white/[0.02] transition-colors border-b border-[#2a2d35]/50 last:border-none">
                <div className="flex items-center gap-3">
                  <span className="w-7 h-7 rounded bg-white/5 flex items-center justify-center text-xs font-bold text-[#ababab]">{String.fromCharCode(65 + i)}</span>
                  <span className="text-sm font-medium">{p.title}</span>
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-xs font-semibold" style={{ color: DIFF_CLR[p.difficulty] }}>{p.difficulty}</span>
                  <span className="text-xs text-[#ababab]">{p.points} pts</span>
                </div>
              </Link>
            ))}
          </div>
