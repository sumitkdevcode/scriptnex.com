...
async function fetchAllPaginatedSlugs(endpoint: string): Promise<SlugItem[]> {
  ...
  let page = 1;
  const maxPages = 20; // Lower safety limit (20 * 100 = 2000 items max) to prevent OOM on cPanel
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // Use current date as fallback so lastModified is accurate for the time the sitemap is generated
  const RUN_DATE = new Date().toISOString();

  // Static high-value pages with strategic priorities
  const staticRoutes: MetadataRoute.Sitemap = [
    { url: baseUrl, lastModified: RUN_DATE, changeFrequency: 'daily', priority: 1.0 },
    { url: `${baseUrl}/problems`, lastModified: RUN_DATE, changeFrequency: 'daily', priority: 0.9 },
    { url: `${baseUrl}/certifications`, lastModified: RUN_DATE, changeFrequency: 'weekly', priority: 0.9 },
    { url: `${baseUrl}/tracks`, lastModified: RUN_DATE, changeFrequency: 'weekly', priority: 0.9 },
    { url: `${baseUrl}/contests`, lastModified: RUN_DATE, changeFrequency: 'daily', priority: 0.8 },
    ...
    { url: `${baseUrl}/refund`, lastModified: RUN_DATE, changeFrequency: 'yearly', priority: 0.3 },
  ];
