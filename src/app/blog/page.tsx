...
export default function BlogPage() {
  ...
  const [tags, setTags] = useState<(BlogPostTag & { posts_count: number })[]>([]);
  ...
  const fetchPosts = useCallback(async (pageNum: number, query: string, tag: string | null, append = false) => {
    try {
      ...
      const newPosts: BlogPostListItem[] = (res as unknown as { data: BlogPostListItem[]; meta?: { last_page?: number } }).data || [];
      const meta = (res as unknown as { meta?: { last_page?: number } }).meta;
    ...
    async function load() {
      try {
        const [featuredRes, tagsRes] = await Promise.all([
          api.get<{ posts: FeaturedPost[] }>('/blog/featured'),
          api.get<{ tags: (BlogPostTag & { posts_count: number })[] }>('/blog/tags'),
        ]);
  ...
  }, [page, search, activeTag, fetchPosts, loadingMore, hasMore]);
          ...
          <div className="blog-featured">
            <div className="blog-featured-grid">
              {featured.slice(0, 3).map((post, i) => (
                ...
                >
                  {post.cover_image ? (
                    <img src={post.cover_image} alt={post.title} className="card-image" />
                  ) : (
                    <div className="blog-no-image card-image">📄</div>
                  )}
                  ...
                  <div className="blog-card-body">
                    ...
                    <div className="blog-card-footer">
                      <div className="blog-card-author">
                        <img src="/logo.png" alt="ScriptNex" className="blog-card-avatar" />
