...
const STATUS_CLR: Record<string, string> = {
  ...
  runtime_error: '#ef4444', compilation_error: '#ef4444', queued: '#64748b', running: '#3b82f6',
...
export default function SubmissionsPage() {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto w-full">
      ...
      <p className="text-[#94a3b8] text-sm mb-8">Your code submission history</p>

      <div className="bg-[#16181d] border border-[#2a2d35] rounded-xl overflow-hidden">
        <div className="grid grid-cols-[1fr_120px_100px_100px_150px] gap-4 px-6 py-3 border-b border-[#2a2d35] text-[10px] uppercase tracking-widest text-[#64748b] font-semibold">
        ...
        {loading ? (
          ...
          subs.map(s => (
            <div key={s.id} className="grid grid-cols-[1fr_120px_100px_100px_150px] gap-4 px-6 py-3.5 items-center border-b border-[#2a2d35]/50 last:border-none hover:bg-white/[0.02]">
              ...
              <span className="text-xs text-[#94a3b8]">{s.language?.name ?? '—'}</span>
              <div className="text-center"><span className="text-[11px] font-semibold capitalize" style={{ color: STATUS_CLR[s.status] || '#64748b' }}>{s.status.replace(/_/g, ' ')}</span></div>
              <div className="text-center text-xs text-[#94a3b8]">{s.runtime_ms ? `${s.runtime_ms}ms` : '—'}</div>
              <div className="text-right text-xs text-[#64748b]">{new Date(s.submitted_at).toLocaleString()}</div>
            </div>
          ))
        )}
      </div>
