  ...
  return (
    <div className="p-8 max-w-4xl mx-auto w-full">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-sm text-[#ababab]">{bookmarks.length} saved</span>
      </div>
      <p className="text-[#ababab] text-sm mb-6">Problems you&apos;ve saved to tackle later.</p>
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-lg text-xs font-semibold uppercase tracking-wider transition-colors ${
              filter === d
                ...
                : 'bg-[#16181d] border border-[#2a2d35] text-[#ababab] hover:text-white'
            }`}
          >
            {d}
          </button>
      ...
      {loading ? (
        ...
        <div className="bg-[#16181d] border border-[#2a2d35] rounded-xl overflow-hidden">
          <div className="grid grid-cols-[1fr_auto_auto_auto] text-[10px] uppercase tracking-widest text-[#ababab] font-semibold px-6 py-3 border-b border-[#2a2d35] gap-4">
            <span>Problem</span>
            <span>Category</span>
          ...
          {filtered.map((b, i) => (
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                <span className="text-xs text-[#ababab] w-6 shrink-0">{i + 1}</span>
                <Link href={`/problems/${b.slug}`} className="font-medium text-sm hover:text-[#00d285] transition-colors truncate">
                  {b.title}
                </Link>
                <span className="text-xs font-semibold shrink-0" style={{ color: DIFF_CLR[b.difficulty] || '#ababab' }}>
                  {b.difficulty}
                </span>
              </div>
              <span className="text-xs text-[#ababab] shrink-0">{b.category?.name || '—'}</span>
              <span className="text-xs text-[#ababab] shrink-0">{b.acceptance_rate ?? '—'}%</span>
              <button
                onClick={() => handleRemove(b.slug)}
                className="text-[#ababab] hover:text-red-400 transition-colors p-1 shrink-0"
