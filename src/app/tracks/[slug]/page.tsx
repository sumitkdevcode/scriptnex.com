  ...
  return (
    <div className="min-h-screen bg-[#0f1115] text-[#f8fafc]">
      ...
      <div className="max-w-5xl mx-auto px-4 pt-0 pb-6">
        <Link href="/tracks" className="text-xs text-[#ababab] hover:text-white mb-2 inline-block">← Back to Tracks</Link>
        <h1 className="text-2xl font-bold mb-1.5">{track.title}</h1>
        <p className="text-[#ababab] text-sm mb-4">{track.description}</p>
        <div className="flex gap-6 text-xs text-[#ababab] mb-6">
          ...
          {modules.map((mod, mi) => (
            <div key={mod.id} className="bg-[#16181d] border border-[#2a2d35] rounded-xl overflow-hidden">
              <div className="px-6 py-4 border-b border-[#2a2d35] flex items-center gap-3">
                ...
                <span className="text-xs text-[#ababab] ml-auto">{mod.lessons.length} lessons</span>
              </div>
              <div className="divide-y divide-[#2a2d35]/50">
                {mod.lessons.map(lesson => (
                  <Link key={lesson.id} href={`/tracks/${track.slug}/${lesson.slug}`} className="flex items-center gap-4 px-6 py-3.5 hover:bg-white/[0.02] transition-colors cursor-pointer">
                    ...
                    <span className="text-xs text-[#ababab]">{lesson.duration_minutes} min</span>
                    <span className="px-2 py-0.5 text-[10px] rounded bg-white/5 text-[#ababab] capitalize">{lesson.type}</span>
                  </Link>
                ))}
              </div>
            </div>
